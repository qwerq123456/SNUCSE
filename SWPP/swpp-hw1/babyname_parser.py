...
def check_filename_existence(func):
    ...
    """
    def inner(self,dirname, year):
        filename = f'{dirname}/{year}.html'
...
class BabynameParser:
    ...
    def __init__(self, dirname, year):
        ...
        self.rank_to_names_tuples = re.findall(r"<td>(\d+)</td> <td>(.+)</td> <td>(.+)</td>", text)
    ...
    def parse(self, parsing_lambda):
        ...
        return [parsing_lambda(t) for t in self.rank_to_names_tuples]
