...
class BabyRecord:
    ...
    def to_csv_record(self):
        ...
        if self.rank_change:
            return f"{self.year},{self.rank},{self.name},{self.gender},{self.rank_change}"
        elif self.rank_change == 0:
            return f"{self.year},{self.rank},{self.name},{self.gender},{self.rank_change}"
        else:
            return f"{self.year},{self.rank},{self.name},{self.gender},"
...
def main():
    ...
    for year in range(year1, year2 + 1):
        ...
        male_records = parser.parse(lambda tuple:BabyRecord(year,int(tuple[0]),tuple[1],"M")) # Parse the male ranks and store them as a list of `BabyRecord` objects.
        female_records = parser.parse(lambda tuple:BabyRecord(year,int(tuple[0]),tuple[2],"F")) # Parse the female ranks and store it as a list of `BabyRecord` objects.
        ...
        for record in male_records:
            if record.name in prev_male_ranking : 
                record.rank_change = (record.rank - prev_male_ranking[record.name])
            else :
                record.rank_change = None
        
        for record in female_records:
            if record.name in prev_female_ranking :
                record.rank_change = (record.rank - prev_female_ranking[record.name])
            else :
                record.rank_change = None

        prev_female_ranking = dict(parser.parse(lambda tuple: (tuple[2], int(tuple[0]))))
        prev_male_ranking = dict(parser.parse(lambda tuple: (tuple[1], int(tuple[0]))))
        
        records += male_records
        records += female_records
